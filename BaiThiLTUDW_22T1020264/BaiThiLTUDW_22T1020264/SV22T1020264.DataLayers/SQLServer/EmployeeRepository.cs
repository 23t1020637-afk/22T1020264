...
namespace SV22T1020264.DataLayers.SQLServer
{
    /// <summary>
    ...
    public class EmployeeRepository : IEmployeeRepository
    {
        ...
        public async Task<string> GetRoleAsync(int employeeID)
        {
            using var connection = new SqlConnection(_connectionString);
            string sql = @"SELECT RoleNames FROM Employees WHERE EmployeeID = @EmployeeID";

            var result = await connection.QueryFirstOrDefaultAsync<string>(sql, new { EmployeeID = employeeID });
            return result;
        }
        ...
        /// <returns></returns>
        public async Task<bool> UpdateRoleAsync(int employeeID, string role)
        {
            using var connection = new SqlConnection(_connectionString);
            string sql = @"
                UPDATE Employees 
